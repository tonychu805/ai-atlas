...
export const NODE_ERA: Record<string, string> = { leading_edge: 'Leading edge (≤3nm)', advanced: 'Advanced (5–7nm)', mature: 'Mature (10–16nm)', legacy: 'Legacy (≥28nm)' }
export const ARCH: Record<string, string> = { planar: 'Planar', finfet: 'FinFET', gaa_nanosheet: 'GAA nanosheet', cfet: 'CFET' }
export const MARKET: Record<string, string> = { datacenter: 'Datacenter', client: 'Client', mobile: 'Mobile', consumer: 'Consumer', automotive: 'Automotive', edge: 'Edge', wearable: 'Wearable' }
export function nodeEra(year: number | null | undefined): string | undefined {
  if (!year) return undefined
  if (year >= 2022) return 'leading_edge'
  if (year >= 2019) return 'advanced'
  if (year >= 2015) return 'mature'
  return 'legacy'
}
...
export const FACET_DEFS: { key: string; label: string; map: Record<string,string>; get: (p: Product) => string | undefined }[] = [
  ...
  { key: 'company_type', label: 'Vendor type',  map: COMPANY,   get: p => p.vendor_supplier?.models?.[0] },
  { key: 'domain',       label: 'Domain',      map: { logic: 'Logic', memory: 'Memory' }, get: p => p.domain },
]
...
export interface Product {
  ...
  vendor_supplier?: { models: string[] } | null
}
